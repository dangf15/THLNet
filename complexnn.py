...
class ComplexConvTranspose2d(nn.Module):
    ...
    def __init__(
        ...
        self.real_conv = nn.ConvTranspose2d(self.in_channels, self.out_channels,kernel_size, self.stride,padding=self.padding,output_padding=output_padding, dilation=self.dilation,groups=self.groups)
        self.imag_conv = nn.ConvTranspose2d(self.in_channels, self.out_channels,kernel_size, self.stride,padding=self.padding,output_padding=output_padding, dilation=self.dilation,groups=self.groups)
        self.complex_axis=complex_axis
...
class ComplexBatchNorm(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        xr, xi = torch.chunk(inputs,2, dim=self.complex_axis)   #for torch_version 1.1.0
